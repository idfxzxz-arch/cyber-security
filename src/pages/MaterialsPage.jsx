      ...
      <div className="grid gap-8 lg:grid-cols-[260px_1fr]">
        <aside className="h-fit rounded-lg border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-24">
          ...
          <div className="mt-4 grid gap-2">
            {categories.map((category) => (
              <button className={`rounded-lg px-3 py-2 text-left text-sm font-semibold transition ${activeCategory === category ? 'bg-blue-950 text-white' : 'text-slate-600 hover:bg-slate-100'}`} key={category} onClick={() => setActiveCategory(category)} type="button">
                {category}
              </button>
            ))}
          </div>
        ...
        <section>
          <div className="mb-8">
            ...
            <h1 className="mt-5 text-3xl font-bold text-slate-950 sm:text-4xl">Daftar Materi Cybersecurity</h1>
